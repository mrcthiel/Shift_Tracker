...
//   	std::string collection_for_XF;
	...
	double fShiftArmBTrack1X,fShiftArmBTrack1Y,fShiftArmBTrack2X,fShiftArmBTrack2Y,fShiftArmFTrack1X,fShiftArmFTrack1Y,fShiftArmFTrack2X,fShiftArmFTrack2Y;	
...
CTPPSRecHitProducer::CTPPSRecHitProducer(const edm::ParameterSet& iConfig)
{
    produces<edm::CTPPSFastRecHitContainer>("CTPPSFastRecHits");
      ...
      fShiftArmBTrack1X = iConfig.getParameter<double>("ShiftArmBTrack1X");
      fShiftArmBTrack1Y = iConfig.getParameter<double>("ShiftArmBTrack1Y");
      fShiftArmBTrack2X = iConfig.getParameter<double>("ShiftArmBTrack2X");
      fShiftArmBTrack2Y = iConfig.getParameter<double>("ShiftArmBTrack2Y");
      fShiftArmFTrack1X = iConfig.getParameter<double>("ShiftArmFTrack1X");
      fShiftArmFTrack1Y = iConfig.getParameter<double>("ShiftArmFTrack1Y");
      fShiftArmFTrack2X = iConfig.getParameter<double>("ShiftArmFTrack2X");
      fShiftArmFTrack2Y = iConfig.getParameter<double>("ShiftArmFTrack2Y");
  ...
  for(MixCollection<PSimHit>::MixItr ihit = simHits->begin();ihit != simHits->end(); ihit++)
	...
	unsigned int detlayerId = simHit->detUnitId(); 
        ...
        if(detlayerId == 2014314496) {
            ...
            float x_trk1 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*um_to_mm) + fShiftArmFTrack1X;
            float y_trk1 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*um_to_mm) + fShiftArmFTrack1Y;
        ...
        if(detlayerId == 2031091712) {
            // Apply position smearing
            float x_trk1 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*um_to_mm) + fShiftArmBTrack1X;
            float y_trk1 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*um_to_mm) + fShiftArmBTrack1Y;
            float z_trk1 = simHit->entryPoint().z();
            // Apply fiducial cuts
            double pos_trk1 = fTrackerInsertion*fBeamXRMS_Trk1+fTrk1XOffset;
            if(x_trk1 >0 || fabs(x_trk1)<pos_trk1 || fabs(x_trk1)>(fTrackerWidth+pos_trk1) || fabs(y_trk1)>fTrackerHeight/2.)outside = true;
        ...
        if(detlayerId == 2014838784) {

            // Apply position smearing
            double x_trk2 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*um_to_mm) + fShiftArmFTrack2X;
            double y_trk2 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*um_to_mm) + fShiftArmFTrack2Y;
        ...
        if(detlayerId == 2031616000) {

            // Apply position smearing
            double x_trk2 = simHit->entryPoint().x()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaX*um_to_mm) + fShiftArmBTrack2X;
            double y_trk2 = simHit->entryPoint().y()+ CLHEP::RandGauss::shoot(engine,0,fHitSigmaY*um_to_mm) + fShiftArmBTrack2Y;
            double z_trk2 = simHit->entryPoint().z();
            // Apply fiducial cuts
            double pos_trk2 = fTrackerInsertion*fBeamXRMS_Trk2+fTrk2XOffset;
            if(x_trk2 >0 || fabs(x_trk2)<pos_trk2 || fabs(x_trk2)>(fTrackerWidth+pos_trk2) || fabs(y_trk2)>fTrackerHeight/2.)outside = true;
