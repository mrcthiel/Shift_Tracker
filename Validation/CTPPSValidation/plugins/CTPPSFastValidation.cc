...
//        edm::Handle< edm::HepMCProduct > EvtHandle ;
        ...
        // Proton Histograms
        TH2F *h_LHCT_xVsy_ARMFs1, *h_LHCT_xVsy_ARMFGPV, *h_LHCT_xVsy_ARMBs1, *h_LHCT_xVsy_ARMBGPV, *h_LHCT_xVsy_GPV;
        TH2F *h_LHCT_xiVst_GPV;
        ...
        TH2F *hMxPPSvsMjj;
        TH1F *h_LHCT_p_GPV, *h_LHCT_pt_GPV, *h_LHCT_pz_GPV, *h_LHCT_py_GPV, *h_LHCT_px_GPV;
        TH1F *h_LHCT_vx_GPV, *h_LHCT_vy_GPV, *h_LHCT_vz_GPV;
...
//    EventKind = Evt->signal_process_id();
    ...
    double x1F = 0., x2F = 0., x1B = 0. , x2B = 0.; 
    //-----------------------------------------------
/*    for(HepMC::GenEvent::particle_const_iterator i=Evt->particles_begin(); i != Evt->particles_end();++i) {
        ...
        if((*i)->status() == 2) {
            Get_t_and_xi(proton,t,xi);
            h_LHCT_xi_GPV->Fill(xi);
            h_LHCT_t_GPV->Fill(t);
            h_LHCT_xiVst_GPV->Fill(xi,t);
            h_LHCT_p_GPV->Fill(proton->P());
            h_LHCT_pt_GPV->Fill(proton->Pt());
            h_LHCT_pz_GPV->Fill(proton->Pz());
            h_LHCT_py_GPV->Fill(proton->Py());
            h_LHCT_px_GPV->Fill(proton->Px());
            h_LHCT_vx_GPV->Fill(vertex.x());
            h_LHCT_vy_GPV->Fill(vertex.y());
...
//                        hxi_ARMB_genreco->Fill(xi,itTracksPPS->xi());
                ...
                if(xiPPSArmF > 0 && xiPPSArmB > 0 && JetsVector_pt.size() == 2){
...
//    }
    ...
    if(PSimHit.isValid()) {
        for (itPSimHit = PSimHit->begin(); itPSimHit != PSimHit->end(); ++itPSimHit) {
            contsimhits++;
            if(itPSimHit->detUnitId() == 2031091712) {
                hxy_ARMB_simdet1->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
                x1B = itPSimHit->entryPoint().x(); 
            }
            if(itPSimHit->detUnitId() == 2031616000) {
                hxy_ARMB_simdet2->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
                x2B = itPSimHit->entryPoint().x(); 
            }
            if(itPSimHit->detUnitId() == 2063597568) hxy_ARMB_simtof->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
            if(itPSimHit->detUnitId() == 2014314496) {
                hxy_ARMF_simdet1->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
                x1F = itPSimHit->entryPoint().x(); 
            }
            if(itPSimHit->detUnitId() == 2014838784) {
                hxy_ARMF_simdet2->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
                x2F = itPSimHit->entryPoint().x(); 
            }
            if(itPSimHit->detUnitId() == 2046820352) hxy_ARMF_simtof->Fill(itPSimHit->entryPoint().x(),itPSimHit->entryPoint().y());
        }
        if (x1B!=0 && x2B!=0 ) {h_x2_x1_B->Fill(x2B-x1B);}
        if (x1F!=0 && x2F!=0 ) {h_x2_x1_F->Fill(x2F-x1F);}
    }
    if(recHits.isValid()) {
...
CTPPSFastValidation::beginJob()
{
    ...
    h_LHCT_xi_GPV = fs->make<TH1F>( "LHCT_xi_GPV" , "LHCT_xiGPV; #xi ; nEvents", 100, 0., .21 );
    h_LHCT_t_GPV  = fs->make<TH1F>( "LHCT_t_GPV" , "LHCT_tGPV; t [GeV^{2}] ; nEvents", 100, .0, 2.0 );
    h_LHCT_xiVst_GPV = fs->make<TH2F>( "LHCT_xiVst_GPV" , "LHCT_xiVstGPV; #xi ; t [GeV^{2}]" , 100, 0., .21 , 100, .0, 2.0 );
    h_LHCT_pt_GPV = fs->make<TH1F>( "LHCT_pt_GPV" , "LHCT_ptGPV; pt [GeV] ; nEvents", 100, .0, 2.0);
    h_LHCT_pz_GPV = fs->make<TH1F>( "LHCT_pz_GPV" , "LHCT_pzGPV; p [GeV] ; nEvents", 1000, 6500.0, 6500.0 );
    h_LHCT_py_GPV = fs->make<TH1F>( "LHCT_py_GPV" , "LHCT_pyGPV; p [GeV] ; nEvents", 1000, -2.0, 2.0 );
    h_LHCT_px_GPV = fs->make<TH1F>( "LHCT_px_GPV" , "LHCT_pxGPV; p [GeV] ; nEvents", 1000, -3.0, 3.0 );
    h_LHCT_p_GPV = fs->make<TH1F>( "LHCT_p_GPV" , "LHCT_pGPV; p [GeV] ; nEvents", 1000, 5200.0, 6500.0 );
    h_LHCT_vx_GPV = fs->make<TH1F>( "LHCT_x_GPV" , "LHCT_xGPV; x [mm] ; nEvents", 100, -1., 1. );
    h_LHCT_vy_GPV = fs->make<TH1F>( "LHCT_y_GPV" , "LHCT_yGPV; y [mm] ; nEvents", 100, -2., 2. );
    h_LHCT_vz_GPV = fs->make<TH1F>( "LHCT_z_GPV" , "LHCT_zGPV; z [mm] ; nEvents", 1000, -200.0, 200.0 );
    const int nXbins = 8;
    double xbin[nXbins+1] = {-18.195, -13.995, -9.795, -7.445, -5.695, -4.535, -3.515, -2.505, -1.695};
    double ybin = 4.2;
    //float zdet1 = 203.8, zdet2 = 212.55, ztof = 215.7;
    hxy_ARMB_simdet1  = fs->make<TH2F>("hxy_ARMB_simdet1", Form("%s, SIM, Z = -203.8, d = 15 #sigma;x [mm] ; y [mm]",_fPhysChannel.c_str()), 100,-30,0.,100,-15,15);
    ...
    hxi_t_ARMF_trk = fs->make<TH2F>( "hxi_t_ARMF_trk" ,Form("%s, CTPPS Tracks, Z > 0;#xi_{reco} ; |t_{reco}| [GeV^{2}]",_fPhysChannel.c_str()),100, 0., .2 , 100, .0, 2.0 );
    hxi_t_ARMB_trk = fs->make<TH2F>( "hxi_t_ARMB_trk" ,Form("%s, CTPPS Tracks, Z < 0;#xi_{reco} ; |t_{reco}| [GeV^{2}]",_fPhysChannel.c_str()),100, 0., .2 , 100, .0, 2.0 );
    ...
    hxi_ARMF_genreco  = fs->make<TH2F>("hxi_ARMF_genreco" , Form("%s, Proton Tracks, Z > 0; #xi_{gen} ; #xi_{reco} ",_fPhysChannel.c_str()), 100, 0., .2 ,  100, 0., .2 );
    hxi_ARMB_genreco  = fs->make<TH2F>("hxi_ARMB_genreco" , Form("%s, Proton Tracks, Z < 0; #xi_{gen} ; #xi_{reco} ",_fPhysChannel.c_str()), 100, 0., .2 ,  100, 0., .2 );
    ht_ARMF_genreco  = fs->make<TH2F>("ht_ARMF_genreco" , Form("%s, Proton Tracks, Z > 0; |t|_{gen} [GeV^{2}] ; |t|_{reco} [GeV^{2}] ",_fPhysChannel.c_str()), 100, .0, 3.0 ,100, .0, 3.0 );
    ht_ARMB_genreco  = fs->make<TH2F>("ht_ARMB_genreco" , Form("%s, Proton Tracks, Z < 0; |t|_{gen} [GeV^{2}] ; |t|_{reco} [GeV^{2}] ",_fPhysChannel.c_str()), 100, .0, 3.0 ,100, .0, 3.0 );
    ...
    h_jet_pt           = fs->make<TH1F>( "h_jet_pt", Form("%s, Jets p_{t}; p_{t} [GeV]; nJets",_fPhysChannel.c_str()), 980,  20.0, 1000. );
    h_x2_x1_B = fs->make<TH1F>( "h_x2_x1_B" , "x2-x1, Z < 0; x2-x1(mm) ; nEvents", 100, -1.,3.);
    h_x2_x1_F = fs->make<TH1F>( "h_x2_x1_F" , "x2-x1, Z > 0; x2-x1(mm) ; nEvents", 100, -1.,3.);
    ...
    for (TH2F *hist : {h_LHCT_xiVst_GPV, hMxPPSvsMjj,
